...
// =========================================================================
    // 11. CRITERIA 3.1: RENDER USER ACCOUNT DATASET ROWS INTO DASHBOARD TABLE
    // =========================================================================
    const adminUsersTbody = document.getElementById('admin-users-tbody');
    
    if (adminUsersTbody) {
        // Fetch the registered users list from local storage
        const currentRegisteredUsers = JSON.parse(localStorage.getItem('debuk_registered_users')) || [];
        
        // Clear old placeholders
        adminUsersTbody.innerHTML = '';
        
        // Add hardcoded admin row reference first
        const adminRow = document.createElement('tr');
        adminRow.style.borderBottom = "1px solid var(--border-color)";
        adminRow.innerHTML = `
            <td style="padding: 1rem 0.5rem; font-family: monospace; color: var(--text-muted);">SYS-7701</td>
            <td style="padding: 1rem 0.5rem; font-weight: 500;">admin@debukshop.com</td>
            <td style="padding: 1rem 0.5rem;"><span style="background: #111; color: #fff; padding: 0.2rem 0.6rem; font-size: 0.75rem; border-radius: 2px; font-weight: 600;">Administrator</span></td>
        `;
        adminUsersTbody.appendChild(adminRow);

        // Map and render all newly registered user accounts dynamically
        if (currentRegisteredUsers.length === 0) {
            const emptyRow = document.createElement('tr');
            emptyRow.innerHTML = `<td colspan="3" style="padding: 2rem 0.5rem; text-align: center; color: var(--text-muted); font-style: italic;">No standard reader accounts registered in current session memory yet.</td>`;
            adminUsersTbody.appendChild(emptyRow);
        } else {
            currentRegisteredUsers.forEach((user, index) => {
                const userRow = document.createElement('tr');
                userRow.style.borderBottom = "1px solid var(--border-color)";
                userRow.innerHTML = `
                    <td style="padding: 1rem 0.5rem; font-family: monospace; color: var(--text-muted);">USR-00${index + 1}</td>
                    <td style="padding: 1rem 0.5rem;">${user.email}</td>
                    <td style="padding: 1rem 0.5rem;"><span style="background: #f3f4f6; color: var(--text-primary); padding: 0.2rem 0.6rem; font-size: 0.75rem; border-radius: 2px; border: 1px solid var(--border-color);">Standard Customer</span></td>
                `;
