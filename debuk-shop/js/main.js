document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================================================
    // 1. GLOBAL STATE MAPPING (CART, TELEMETRY, & ADMIN PERSISTENCE MATRICES)
    // =========================================================================
    ...
    let deletedBaselineIds = JSON.parse(localStorage.getItem('debuk_deleted_baseline_ids')) || [];
    let priceOverrides = JSON.parse(localStorage.getItem('debuk_price_overrides')) || {};
    ...
    // =========================================================================
    // 2. ENHANCED AUTHENTICATION MODAL ENGINE
    // =========================================================================
    ...
    document.addEventListener('click', (e) => {
        if (e.target && e.target.id === 'auth-nav-btn') {
            e.preventDefault();
            if (currentUser) {
    ...
    if (modalAuthForm) {
        modalAuthForm.addEventListener('submit', (e) => {
            ...
            if (isRegisterMode) {
                ...
                alert("Account created successfully!\nYou can now sign in using your reader credentials.");
                resetAuthModalForm();
                modalAuthForm.reset();
                return;
            }
            ...
            if (emailValue === 'admin@debukshop.com') {
                if (passwordValue === '1234') {
                    ...
                    syncAuthNavbarUI();
                    closeAuthModal();
                    alert("Administrative Workspace Unlocked.\nRedirecting directly to the performance center.");
    ...
    function checkAuthenticationGuard() {
        if (!currentUser) {
            alert("Authentication Required:\nPlease log in to access marketplace features.");
            openAuthModal();
            return false;
        }
        return true;
    ...
    // =========================================================================
    // 5. MANAGEMENT DASHBOARD NEON GRAPH ENGINE
    // =========================================================================
    ...
    if (dashboardMainView) {
        if (currentUser !== 'admin') {
            dashboardMainView.innerHTML = `
                <div class="empty-cart-message" style="grid-column: 1/-1; padding: 8rem 0; text-align:center;">
                    <h2 style="color:var(--text-primary); margin-bottom:1rem;">Access Prohibited</h2>
                    <p style="color:var(--text-muted); margin-bottom:1.5rem;">Elevated administrative permissions are required to inspect bookstore operational logs.</p>
                    <button class="btn-submit-contact" id="verify-admin-btn" style="max-width:300px; margin:0 auto; padding:0.75rem 2rem;">Verify Administrative Session</button>
                </div>`;
            document.getElementById('verify-admin-btn').addEventListener('click', () => { openAuthModal(); });
            return;
        }
        ...
        counters.forEach(counter => {
            ...
            const countUp = () => {
                ...
                counter.textContent = counter.id === 'stat-revenue' ? `RM${currentValue.toLocaleString()}` : currentValue.toLocaleString();
                if (frame < totalFrames) { requestAnimationFrame(countUp); } 
                else { counter.textContent = counter.id === 'stat-revenue' ? `RM${targetValue.toLocaleString()}` : targetValue.toLocaleString(); }
            };
        ...
        if (ctxLine && typeof Chart !== 'undefined') {
            ...
            const updateLiveTextBanner = () => {
                if (subtitleText) {
                    subtitleText.innerHTML = `Live Activity • <span style="color:#00f2fe; font-weight:600;">${savedTraffic[targetedArrayDayIndex]} Unique Visitors Today</span>`;
                }
            };
            ...
            const liveVisitorChart = new Chart(ctxLine, {
                ...
                data: {
                    ...
                    datasets: [{
                        data: savedTraffic,
                        borderColor: '#00f2fe', borderWidth: 2, pointBackgroundColor: '#00f2fe', tension: 0.35, fill: true,
                        backgroundColor: (c) => {
                            const chart = c.chart; const {ctx, chartArea} = chart; if (!chartArea) return null;
                            const g = ctx.createLinearGradient(0, chartArea.top, 0, chartArea.bottom);
                            g.addColorStop(0, 'rgba(0, 242, 254, 0.15)'); g.addColorStop(1, 'rgba(0, 242, 254, 0.00)'); return g;
                        }
                    }]
                },
                options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false }, ticks: { color: '#9ca3af' } }, y: { min: 0, grid: { color: '#1e293b' }, border: { dash: [5, 5] }, ticks: { color: '#9ca3af' } } } }
            });

            setInterval(() => {
                savedTraffic[targetedArrayDayIndex] += Math.floor(Math.random() * 2) + 1;
                localStorage.setItem('debuk_visitor_history', JSON.stringify(savedTraffic));
        ...
        if (ctxBar && typeof Chart !== 'undefined') {
            new Chart(ctxBar, {
                ...
                data: {
                    ...
                    datasets: [{ label: 'Units Distributed', data: [24, 38, 45, 18, 31], backgroundColor: '#00f2fe', hoverBackgroundColor: '#7000ff', borderRadius: 2, barThickness: 20 }]
                },
                options: { responsive: true, maintainAspectRatio: false, indexAxis: 'y', plugins: { legend: { display: false } }, scales: { x: { min: 0, grid: { color: '#1e293b' }, border: { dash: [5, 5] }, ticks: { color: '#9ca3af' } }, y: { grid: { display: false }, ticks: { color: '#9ca3af' } } } }
            });
        }
    }

    // =========================================================================
    // 6. CATALOGUE INTERACTIVE REGISTRY MATRIX 
    // =========================================================================
    const baselineBooks = [
        ...
        { id: 11, title: "The Great Gatsby", author: "F. Scott Fitzgerald", genre: "Fiction", price: 10.00, img: "https://covers.openlibrary.org/b/isbn/9780743273565-L.jpg", desc: "A brilliant, tragic autopsy of the promises wrapped inside the American Dream." },
    ...
    let activeBaselineBooks = baselineBooks
        .filter(b => !deletedBaselineIds.includes(b.id))
        .map(b => { if (priceOverrides[b.id]) b.price = priceOverrides[b.id]; return b; });

    let activeCustomBooks = customizedAdminBooks
        .filter(b => !deletedBaselineIds.includes(b.id)) 
        .map(b => { if (priceOverrides[b.id]) b.price = priceOverrides[b.id]; return b; });

    const bookDatabase = [...activeBaselineBooks, ...activeCustomBooks];
    ...
    if (catalogueGrid) {
       function displayBooks(booksToRender) {
            ...
            booksToRender.forEach(book => {
                ...
                if (currentUser === 'admin') {
                    footerControlsHTML = `
                        <div class="admin-actions-flex">
                            <button class="admin-edit-price-btn" data-id="${book.id}">Edit</button>
                            <button class="admin-remove-book-btn" data-id="${book.id}">Remove</button>
                        </div>`;
                } else {
                    footerControlsHTML = `<button class="add-to-cart-btn" data-id="${book.id}">Add To Cart</button>`;
                }
                ...
                card.innerHTML = `
                    ...
                    <div class="card-details">
                        ...
                        <div class="card-footer-flex"><span class="book-price">RM${book.price.toFixed(2)}</span>${footerControlsHTML}</div>
                    </div>
                `;
        ...
        function bindAdminControlEvents() {
            document.querySelectorAll('.admin-edit-price-btn').forEach(button => {
                button.addEventListener('click', (e) => {
                    const bookId = parseInt(e.target.getAttribute('data-id'), 10);
                    const targetedBook = bookDatabase.find(b => b.id === bookId);
                    
                    const inputPrice = prompt(`Enter new retail price for "${targetedBook.title}":`, targetedBook.price);
                    if (inputPrice !== null) {
                        const parsedPrice = parseFloat(inputPrice);
                        if (!isNaN(parsedPrice) && parsedPrice > 0) {
                            priceOverrides[bookId] = parsedPrice;
                            localStorage.setItem('debuk_price_overrides', JSON.stringify(priceOverrides));
                            
                            let customBooks = JSON.parse(localStorage.getItem('debuk_custom_books')) || [];
                            let targetCB = customBooks.find(b => b.id === bookId);
                            if (targetCB) {
                                targetCB.price = parsedPrice;
                                localStorage.setItem('debuk_custom_books', JSON.stringify(customBooks));
                            }
                            
                            alert("Price updated successfully!");
                            window.location.reload();
                        } else {
                            alert("Please provide a valid processing retail value greater than 0.");
                        }
                    }
                });
            });

            document.querySelectorAll('.admin-remove-book-btn').forEach(button => {
                button.addEventListener('click', (e) => {
                    const bookId = parseInt(e.target.getAttribute('data-id'), 10);
                    const targetedBook = bookDatabase.find(b => b.id === bookId);
                    
                    if (confirm(`Are you sure you want to remove "${targetedBook.title}" from the storefront catalogue entirely?`)) {
                        const isCustomItem = customizedAdminBooks.some(b => b.id === bookId);
                        if (isCustomItem) {
                            customizedAdminBooks = customizedAdminBooks.filter(b => b.id !== bookId);
                            localStorage.setItem('debuk_custom_books', JSON.stringify(customizedAdminBooks));
                        } else {
                            deletedBaselineIds.push(bookId);
                            localStorage.setItem('debuk_deleted_baseline_ids', JSON.stringify(deletedBaselineIds));
                        }
                        alert("Masterwork successfully removed from database pipelines.");
                        window.location.reload();
                    }
                });
            });
            ...
            document.querySelectorAll('.add-to-cart-btn').forEach(button => {
                button.addEventListener('click', (e) => {
                    ...
                    if (completeBookData) {
                        ...
                        updateCartBadge();
                        button.textContent = "Added ✓"; button.style.backgroundColor = "var(--accent-color)"; button.style.color = "var(--bg-primary)";
        ...
        const deepSearchQuery = urlParams.get('search');

        if (deepSearchQuery) {
            const filteredBooks = bookDatabase.filter(book => 
                book.title.toLowerCase().includes(deepSearchQuery.toLowerCase())
            );
    ...
    function displayCartPage() {
        ...
        document.getElementById('summary-subtotal').textContent = `RM${subtotalAccumulator.toFixed(2)}`;
        document.getElementById('summary-total').textContent = `RM${subtotalAccumulator.toFixed(2)}`;
        bindCartModifiers();
    }
    ...
    function displayPaymentPage() {
        ...
        cart.forEach(item => {
            ...
            itemRow.innerHTML = `<img src="${item.img}" alt="${item.title}"><div class="mini-item-meta"><h4>${item.title}</h4><p>Qty: ${item.quantity}</p></div><span class="mini-item-price">RM${(item.price * item.quantity).toFixed(2)}</span>`;
            checkoutItemsList.appendChild(itemRow);
        });
        document.getElementById('checkout-subtotal').textContent = `RM${checkoutSubtotal.toFixed(2)}`;
        document.getElementById('checkout-total').textContent = `RM${checkoutSubtotal.toFixed(2)}`;
    }
    ...
    if (cardNumInput) {
        cardNumInput.addEventListener('input', (e) => {
            ...
            let parts = [];
            for (let i=0, len=value.length; i<len; i+=4) { parts.push(value.substring(i, i+4)); }
    ...
    if (paymentForm) {
        paymentForm.addEventListener('submit', (e) => {
            ...
            const receiptBillingName = document.getElementById('receipt-billing-name');
            if(receiptBillingName) receiptBillingName.textContent = billName;
            
            const receiptBillingEmail = document.getElementById('receipt-billing-email');
            if(receiptBillingEmail) receiptBillingEmail.textContent = billEmail;

            const receiptCost = document.getElementById('receipt-cost');
            if(receiptCost) receiptCost.textContent = `RM${currentRevenueAmount.toFixed(2)}`;

            const receiptBooksList = document.getElementById('receipt-books-list');
            if(receiptBooksList) {
                receiptBooksList.innerHTML = ''; 
                cart.forEach(item => {
                    ...
                    row.innerHTML = `<span>${item.quantity}x ${item.title}</span><span style="color:var(--text-primary)">RM${(item.price * item.quantity).toFixed(2)}</span>`;
                    receiptBooksList.appendChild(row);
                });
            }

            const receiptTracking = document.getElementById('receipt-tracking-id');
            if(receiptTracking) receiptTracking.textContent = 'PL-' + Math.floor(100000 + Math.random() * 900000) + '-MY';
    ...
    if (successConfirmBtn) {
        successConfirmBtn.addEventListener('click', () => { window.location.href = 'index.html'; });
    }
    ...
    // =========================================================================
    // 10. ADMINISTRATIVE INVENTORY MANAGEMENT ENGINE
    // =========================================================================
    ...
    if (inventoryMainView) {
        if (currentUser !== 'admin') {
            inventoryMainView.innerHTML = `
                <div class="empty-cart-message" style="grid-column: 1/-1; padding: 8rem 0; text-align:center;">
                    <h2 style="color:var(--text-primary); margin-bottom:1rem;">Access Prohibited</h2>
                    <p style="color:var(--text-muted); margin-bottom:1.5rem;">Elevated credential permissions are required to append items to the active book index logs.</p>
                    <button class="btn-submit-contact" id="verify-inventory-admin-btn" style="max-width:300px; margin:0 auto; padding:0.75rem 2rem; cursor:pointer;">Verify Administrative Session</button>
                </div>`;
            
            document.getElementById('verify-inventory-admin-btn').addEventListener('click', () => { openAuthModal(); });
            return;
        }
    }
    ...
    // =========================================================================
    // 11. DASHBOARD REGISTERED READERS DATASET RENDERING TRACE BOARD
    // =========================================================================
    ...
    if (adminUsersTbody) {
        ...
        adminRow.innerHTML = `
            <td style="padding: 1rem 1.5rem; font-family: monospace; color: var(--text-muted);">SYS-7701</td>
            <td style="padding: 1rem 1.5rem; font-weight: 500;">admin@debukshop.com</td>
            <td style="padding: 1rem 1.5rem;"><span style="background: var(--accent-color); color: var(--bg-primary); padding: 0.2rem 0.6rem; font-size: 0.75rem; border-radius: 2px; font-weight: 600;">Administrator</span></td>
        `;
        adminUsersTbody.appendChild(adminRow);

        if (registeredUsers.length === 0) {
            const emptyRow = document.createElement('tr');
            emptyRow.innerHTML = `<td colspan="3" style="padding: 2rem 1.5rem; text-align: center; color: var(--text-muted); font-style: italic;">No standard reader accounts registered in current session memory yet.</td>`;
            adminUsersTbody.appendChild(emptyRow);
        } else {
            registeredUsers.forEach((user, index) => {
                const userRow = document.createElement('tr');
                userRow.innerHTML = `
                    <td style="padding: 1rem 1.5rem; font-family: monospace; color: var(--text-muted);">USR-00${index + 1}</td>
                    <td style="padding: 1rem 1.5rem;">${user.email}</td>
                    <td style="padding: 1rem 1.5rem;"><span style="background: var(--bg-secondary); color: var(--text-primary); padding: 0.2rem 0.6rem; font-size: 0.75rem; border-radius: 2px; border: 1px solid var(--border-color);">Standard Customer</span></td>
                `;
                adminUsersTbody.appendChild(userRow);
            });
        }
    }

    // =========================================================================
    // 12. RETRACTABLE AUTO-HIDE HEADER VIEWPORT CURSOR SENSORS
    // =========================================================================
    const masterNavbar = document.querySelector('.navbar');
    if (masterNavbar) {
        masterNavbar.classList.add('navbar-visible');
        setTimeout(() => { if (authModal && !authModal.classList.contains('modal-open')) masterNavbar.classList.remove('navbar-visible'); }, 2000);

        window.addEventListener('mousemove', (event) => {
            if (authModal && authModal.classList.contains('modal-open')) { masterNavbar.classList.remove('navbar-visible'); return; }
            if (event.clientY <= 60) { masterNavbar.classList.add('navbar-visible'); } 
            else if (event.clientY > 120 && !masterNavbar.classList.contains('mobile-active')) { masterNavbar.classList.remove('navbar-visible'); }
        });
        masterNavbar.addEventListener('mouseenter', () => { if (authModal && authModal.classList.contains('modal-open')) return; masterNavbar.classList.add('navbar-visible'); });
    }
});